...
<main class="contenedor-producto sombra">
    <a class="titulos-body">Carrito</a>
    <table class="table">
    <thead>
        <tr>
            <th>Producto</th>
    ...
    <tbody>
        ...
        ?>
        <tr>
            <td><?php echo str_replace("`", "'", $item["name"]) . "<br> por " . $item["artist"]; ?></td>
            <td><?php echo 'MX$'.$item["price"]; ?></td>
            <td><?php echo $item["type"]; ?></td>
            <td><input type="number" class="form-control text-center" value="<?php echo $item["qty"]; ?>" onchange="updateCartItem(this, '<?php echo $item["rowid"]; ?>')"></td>
            <td><?php echo 'MX$'.$item["subtotal"]; ?></td>
            <td>
            <a href="cartAction.php?action=removeCartItem&id=<?php echo $item["rowid"]; ?>" onclick="return confirm('Are you sure?')">Eliminar</a>
            </td>
        </tr>
        <?php } }else{ ?>
        <tr><td colspan="5"><p>Tu carrito está vacío. <br>> Añade artículos para continuar.</p></td>
        <?php } ?>
    </tbody>
    <tfoot>
        <tr>
            ...
            <td class="text-center"><strong>Total <?php echo 'MX$'.$cart->total(); ?></strong></td>

...
<head>
    ...
    <title><?php echo $nombre ?> por <?php echo $artista ?> | BandUp</title>
...
<body>
    ...
    <div class="logo-responsivo">
        <a href="https://bandup.monkeydevs.mx">
            <img src="/bandup/img/BandUp.svg" width="160">
        </a>
    ...
    <main class="contenedor-producto sombra">
                <?php
                $query = $link->query("SELECT * FROM productos WHERE id = 10000");
                if($query->num_rows > 0){ 
                    while($row = $query->fetch_assoc()) {
                ?>
                        <div class="img-producto">
                            <?php echo '<img src="data:image/png;base64,'.base64_encode($row['img']).'" class="alinear-izquierda" width="400"/>';?>
                            </div>
                                <div class="contenido-producto">
                                    <a class="titulos-producto"><?php echo $row["nombre"]; ?></a>
                                    <a class="titulos-producto artista">por <?php echo $row["artista"]; ?></a>
                                    <a class="titulos-producto artista"><?php echo 'MX$'.$row["precioV"]; ?></a>
                                    <a class="titulos-producto descripcion"><?php echo $row["descripcion"]; ?></a>
                                    <button class="registro anadir-carrito" href="cartAction.php?action=addToCart&id=<?php echo $row["id"]; ?>">Añadir al carrito</button>
                                    <a href="cartAction.php?action=addToCart&id=<?php echo $row["id"]; ?>">Añadir al carrito</a>
                                    </div>
                            <!--div class="">
                                <p class="">
                                </p>

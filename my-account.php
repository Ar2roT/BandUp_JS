<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $imagenE = file_get_contents($imagen);
            ...
            $sql = "UPDATE usuarios SET img = ? WHERE id = ".$_SESSION["id"];
...
<body>
    ...
    <nav class="nav-inf">
        ...
        <main class="contenedor sombra">
            <a class="titulos-body">Mi cuenta</a>
            <?php
                $query = $link->query("SELECT * FROM usuarios WHERE id =".$id);
                if($query->num_rows > 0){ 
                    while($row = $query->fetch_assoc()) {
                ?>
                        <div class="img-producto">
                            <div class="alinear-derecha">
                            <?php echo '<img src="data:image/png;base64,'.$row["img"].'" class="img-portada" width="400" height="400"/>';?>
                            <?php
                }}
                ?>
                    </div>
            ...
            <form enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype="multipart/form-data" method="post">
                <input type="file" name="imagen">
                <input type="submit" value="Subir">
            </form>
            <a href="reset-password" class="btn btn-warning">Cambia tu contraseña</a><br>
            <a href="logout" class="btn btn-danger">Cerrar sesión</a><br>
            <a href="/admin/product-insert" class="<?php echo $style ?>">Añadir productos</a><br>
            <a href="/admin/product-edit" class="<?php echo $style ?>">Editar productos</a><br>
        </main>
